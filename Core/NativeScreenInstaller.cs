...
namespace PrizeTracker.Core
{
    ...
    internal class NativeScreenInstaller : MonoBehaviour
    {
        ...
        private NativeHistoryScreen _screen;
        ...
        private void Update()
        {
            ...
            catch (Exception e)
            {
                ...
                Plugin.Log.LogWarning("native history screen not installed (" + e.Message +
                                      "); falling back to the overlay screen.");
            }
        }
        ...
        private void TryInstall()
        {
            ...
            if (_attempts++ > 60)
            {
                if (_attempts == 62) Plugin.Log.LogWarning(
                    "native history screen: menu is up but the expected objects never appeared; " +
                    "keeping the overlay screen.");
                return;
            }
            ...
            var go = new GameObject("PrizeTrackerMatchHistoryScreen", typeof(RectTransform));
            go.transform.SetParent(parent, false);
            NativeHistoryScreen.Stretch((RectTransform)go.transform);

            _screen = go.AddComponent<NativeHistoryScreen>();
            ...
            // ---- the tab ---------------------------------------------------
            WireTab(button);
            Plugin.Log.LogWarning("native history screen installed under " + parent.name +
        ...
        private void Navigate()
        {
            try
            {
                ...
                {
                    Plugin.Log.LogWarning("history screen declined to open (currentScreen=" +
                ...
                Plugin.Log.LogInfo("match history screen opened; parent=" +
                                   _screen.transform.parent.name);
            }
            catch (Exception e)
            {
                Plugin.Log.LogWarning("history tab navigation failed: " + e);
            }
        }
        ...
        private void RunSelfTest()
        {
            try
            {
                Plugin.Log.LogWarning("self-test: opening the history screen (attempt " +
                                      (_selfTestTries + 1) + ")...");
                Navigate();
